...
///
/// \file   ulf/mx1bin/message/acceleration.hpp
...
struct Acceleration {
  struct Reply {
    using Head = detail::ReplyHead<Command::Acceleration>;
  ...
  using Head = detail::Head<Command::Acceleration>;
  Head head{.info = bitfields::Info{FrameType::Short,
                                    MessageType::Primary,
                                    Sender::CommandStation,
                                    StationType::MX1}};
  ...
  template<Encoder E>
  E encode(E e) const {
    return head.encode(e).uint16(cAdr).uint8(cAzBz);
  }
  template<Decoder D>
  static std::expected<Acceleration, std::errc> decode(D& d) {
    auto const head{Head::decode(d)};
    if (!head || !d.has_at_least(sizeof(cAdr) + sizeof(cAzBz)))
      return std::unexpected(std::errc::invalid_argument);
